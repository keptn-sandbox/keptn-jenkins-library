    ...
    timeout(time: waitTime, unit: 'MINUTES') {
        script {
            waitUntil {
                ...
                if (response.status == 404 || response.content.contains("No Keptn sh.keptn.event.evaluation.finished event found for context") || response.content.contains("{"events":[],"pageSize":20}")  ) {
                    sleep 10
                    return false
                } else {
